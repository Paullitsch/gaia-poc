...
Each island evolves neuromodulated agents. Migration shares the best
biologically-plausible learners across the network.

This is the closest experiment to the full GAIA vision:
- Local learning rules (neuromodulation) instead of global gradients
- Decentralized evolution (islands) instead of centralized training
- Emergent intelligence from simple, local interactions
"""
...
class IslandCMAES:
    def __init__(self, n_params, sigma0=0.5):
        ...
        self.C_diag = np.ones(n_params)  # Always diagonal for neuromod (larger params)
...
def run(params=None, device="cpu", callback=None):
    ...
    print(f"🧬🏝️  Neuromod Island Model: {n_islands} islands")
    ...
    labels = [configs[i % len(configs)]["label"] for i in range(n_islands)]
    ...
    while total_evals < max_evals:
        ...
        if n_workers > 1:
            ...
            all_fitnesses = np.array([evaluate_neuromod(policy, c, eval_episodes) for _, c, _ in all_candidates])
        ...
        print(f"Gen {gen:4d} | Best: {max(island_bests):8.1f} | Ever: {best_ever:8.1f} | "
              f"Mean: {np.mean(all_fitnesses):8.1f} | σ̄: {avg_sigma:.4f} | Evals: {total_evals:6d} | {elapsed:6.1f}s {solved}")

        if best_ever >= 200:
            print(f"\n🎉🧬🏝️  SOLVED with Neuromod Island Model! Score: {best_ever:.1f}")
            break
